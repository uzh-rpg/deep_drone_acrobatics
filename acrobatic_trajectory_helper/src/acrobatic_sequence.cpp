...
bool AcrobaticSequence::appendMattyLoop(const int n_loops, const double& circle_velocity, const double& radius,
  ...
  printf("Enter state of loop maneuver: Pos: %.2f, %.2f, %.2f | Vel: %.2f, %.2f, %.2f\n",
         init_state.position.x(),
         init_state.position.y(),
         init_state.position.z(),
         init_state.velocity.x(),
         init_state.velocity.y(),
         init_state.velocity.z());
...
//  printf("Maximum speed: %.3f, current speed: %.3f\n", 1.1*circle_velocity, start_state.velocity.norm());
  ...
  trajectory_generation_helper::heading::addConstantHeadingRate(0.0,
                                                                M_PI,
                                                                &enter_trajectory);
...
//                                                            &enter_trajectory);
  ...
  const Eigen::Vector3d end_pos_P = circle_center_offset_end; //Eigen::Vector3d(circle_center_offset.x(),
  //scircle_center_offset.y(),-circle_center_offset.z()); // nice breaking forward flip
  quadrotor_common::TrajectoryPoint end_state;
  end_state.position = q_W_P * end_pos_P + circle_center;
  end_state.velocity = q_W_P * Eigen::Vector3d(circle_velocity, 0.0, 0.0);
...
//  trajectory_generation_helper::heading::addConstantHeading(desired_heading_loop,
//                                                            &exit_trajectory);
  trajectory_generation_helper::heading::addConstantHeadingRate(M_PI,
                                                                2 * M_PI,
                                                                &exit_trajectory);
...
bool AcrobaticSequence::appendBarrelRoll(const int n_loops, const double& circle_velocity, const double& radius,
  ...
  printf("Enter state of cork screw maneuver: Pos: %.2f, %.2f, %.2f | Vel: %.2f, %.2f, %.2f\n",
         init_state.position.x(),
         init_state.position.y(),
         init_state.position.z(),
         init_state.velocity.x(),
  ...
  quadrotor_common::Trajectory circle_trajectory =
      trajectory_generation_helper::circles::computeVerticalCircleTrajectoryCorkScrew(
...
//  const Eigen::Vector3d end_pos_P = Eigen::Vector3d(4.0, 0.0, 0.5); // nice breaking forward flip
  quadrotor_common::TrajectoryPoint end_state;
//  end_state.position = q_W_P * end_pos_P + circle_center;
//  end_state.velocity = q_W_P * Eigen::Vector3d(circle_velocity, 0.0, 0.0);
//  end_state.position = init_state.position + Eigen::Vector3d(10.0, 0.0, 0.0);
